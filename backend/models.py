...
# Predictor class for Flask API
class NFLAwardPredictor:
    def __init__(self):
        import joblib
        self.model = joblib.load('nfl_mvp_model.pkl')
        self.features = [
            'wins', 'qbr_total', 'epa_total', 'epa_per_play',
            'qb_plays', 'sacks', 'passing_tds', 'interceptions', 
            'passing_yards', 'passer_rating', 'rushing_yards', 'rushing_tds'
    ...
    def predict(self, input_data):
        import numpy as np
        # input_data should be a dict with keys matching self.features
        X = np.array([[input_data.get(f, 0) for f in self.features]])
        prediction = self.model.predict(X)
        return {'mvp': int(prediction[0])}